...
class ApiController(base.BaseController):
    ...
    def mostDownloadedDataset(self, ver=None):
        ...
        itemList = get_action('package_search')(context, data_dict)
        results = []
        for item in itemList.get('results'):
            results.append({'count': item.get('tracking_summary').get('recent'),'title': item.get('title'), 'name': item.get('name')})
        return self._finish_ok(results)
    ...
    def mostRecentDataset(self, ver=None):
        ...
        results = []
        for item in itemList.get('results'):
            results.append({'title': item.get('title'), 'name': item.get('name')})
        return self._finish_ok(results)

    def getDataCount(self, ver=None):
        data_dict = {
                'q': 'state: active && dataset_type: dataset'
        }
        ...
        while (total>procesados):
           for item in itemList.get('results'):
              for item in item.get('resources'):
                    if item.get('format').upper() == 'XLS':
                       resourceCount += 4
                    elif item.get('format').upper() == 'URL':
                       resourceCount += 5
                    else :
                       resourceCount += 1

              #resourceCount += len(item.get('resources'))
           procesados += len(itemList.get('results'))
        ...
        #results.append({'datasetCount': len(itemList.get('results')), 'resourceCount':  resourceCount})
        results.append({'datasetCount': procesados, 'resourceCount':  resourceCount})
        #results.append({'datasetCount': itemList['count']})
