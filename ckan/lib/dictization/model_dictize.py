...
def package_dictize(pkg, context):
    ...
    q = select([extra_rev]).where(extra_rev.c.package_id == pkg.id).where(~extra_rev.c.key.ilike(stringLike))
    result = _execute_with_revision(q, extra_rev, context)
    result_dict["extras"] = extras_list_dictize(result, context)
    stringLike = '%_IAEST_%'
    extra_rev = model.extra_revision_table
    q = select([extra_rev]).where(extra_rev.c.package_id == pkg.id).where(extra_rev.c.key.ilike(stringLike))
    result2 = _execute_with_revision(q, extra_rev, context)
    result_dict["extrasIAEST"] = extras_list_dictize(result2, context)
    #tracking
