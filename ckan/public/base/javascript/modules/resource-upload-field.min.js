    ...
    options: {
      ...
      template: [
       ...
       // '<i class="ckan-icon ckan-icon-link-plugin"></i>',
        '<input type="file" class="jfilestyle" data-input="false" data-icon="false" data-buttonText="Seleccionar fichero local" data-theme="blue" autocomplete="off" />',
...
//        '<input id="field-resource-type-upload" type="radio" name="resource_type" value="file.upload" />',
  //      '<label class="radio inline" for="field-resource-type-upload"></label>',
    ...
    lookupMetadata: function (key, data) {
      ...
      //var urlUploadedResource = $.parseJSON(request.responseText)['_location'];
...
//      var text = org.value;
      var text = "general";
      if (availableOrganizations > 1) {
        var idx = $("#owner_org")[0].selectedIndex;

        if (window.confirm("El recurso será asignado a la organización " + $("#owner_org")[0][idx].text+ ". En caso de no ser correcto, cancele y asigne la organización correcta previamente")) {
        } else {
          return '';
        }
	text = organizationList[$("#owner_org")[0][idx].text];
      } else {
    ...
    _onUploadDone: function (event, data) {
      ...
      if (result && !(jQuery.isPlainObject(result) && result.error)) {      
        $("#up" + this.el[0].id).val("http://opendata.aragon.es/catalogo/opendata/" + data.key);

        // it isn't necessary to spend time calculating this, because we only need URL and it's known
