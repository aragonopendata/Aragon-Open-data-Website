...
def url_with_params(url, params):
    ...
    log.error("[M] url_with_params t4 params - " + str(params))
...
def search_url(params, package_type=None):
    if not package_type or package_type == 'catalogo':
        ...
        log.error("[M] t2 url - " + str(url))
        log.error("[M] t3 params - " + str(params))
...
class PackageController(base.BaseController):
    ...
    def search(self):
        ...
        def pager_url(q=None, page=None):
            ...
            log.error("[M] 4 params" + str(params))
            return search_url(params, package_type)

        #[M]
        c.search_url_params = urlencode(_encode_params(params_nopage))
        #c.search_url_params = params_nopage[M] url_with_params t4 params
        log.error("[M] 12 c.search_url_params" + str(c.search_url_params))
        try:
            c.fields = []
            ...
            log.error("[M] 5 default_facet_titles - " + str(default_facet_titles))
            ...
            log.error("[M] 6 data_dict" + str(data_dict))
            query = get_action('package_search')(context, data_dict)
            c.sort_by_selected = query['sort']
            log.error("[M] 7 c.sort_by_selected - " + str(c.sort_by_selected))
            c.page = h.Page(
            ...
            log.error("[M] 8 c.page - " + str(c.page))
            c.facets = query['facets']
            c.search_facets = query['search_facets']
            c.page.items = query['results']
            log.error("[M] 9 c.facets - " + str(c.facets))
            log.error("[M] 10 c.search_facets - " + str(c.search_facets))
            log.error("[M] 11 c.page.items - " + str(c.page.items))
