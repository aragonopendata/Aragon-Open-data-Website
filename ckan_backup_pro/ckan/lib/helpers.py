...
def resource_url_for(resource_dict, format):
    ''' returns the resource download url for format passed (xml, json or csv)'''
    name = re.sub("\.\w{3,4}$", "", resource_dict.get('name')) + "." + format
    print name
    return string.join(('/render', 'resource', name), '/')

def get_package_org(id):
        pkg = model.Package.get(id)
        if pkg.owner_org:
            organizacion =  pkg.owner_org
            group = model.Group.get(organizacion)
            return group.name
        else:
            return "general"
...
#Se crean las funciones helper para el dao
