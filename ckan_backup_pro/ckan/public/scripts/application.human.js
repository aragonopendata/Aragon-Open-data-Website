...
CKAN.View.ResourceEditor = Backbone.View.extend({
  initialize: function() {
    ...
    // Tabs for adding resources - VISTA
    new CKAN.View.ResourceAddUrlVista({
      collection: this.collection,
      el: this.el.find('.js-add-url-form-vista'),
      mode: 'file'
    });
    new CKAN.View.ResourceAddUrl({
      collection: this.collection,
...
/* ============================================== */
/* == Backbone View: Add VISTA resource by URL == */
/* ============================================== */
CKAN.View.ResourceAddUrlVista = Backbone.View.extend({
  initialize: function(options) {
    _.bindAll(this, 'clickSubmit');
  },
  ...
  clickSubmit: function(e) {
     ...
     this.el.find('input[name="add-resource-save"]').addClass("disabled");
     var urlVal = "";
     var formatVal = this.el.find('select[name="formato"]').val();
     ...
     if (formatVal != "-1"){
	     ...
	     $.ajax({
		      url: CKAN.SITE_URL + '/guardarVista/' + vista,
		      async: false,
		      data:"filtro=" + filtro + ";vista=" + vista + ';formato=' + formatVal,
		      dataType: 'text',
		      success: function(data) {
			...
			urlVal = data;
		      ...
		      error: function(jqXHR, textStatus, errorThrown) {
			alert("A1-No se ha podido llevar a cabo la consulta");
		      }
		 ...
		 dataType: 'json',
		 error: function(){
		   newResource.set({url: urlVal, resource_type: 'vista', format: formatVal});
		   self.collection.add(newResource);
		   self.resetForm();
		 },
		 success: function(data){
	       ...
	       newResource.set({url: urlVal, format: formatVal, resource_type: 'vista', last_modified: lastmod});
	       this.collection.add(newResource);
	       this.resetForm();
	     }
...
ISODateString: function(d) {
  ...
  resetForm: function() {
	...
	document.getElementById("guardadoDiv").innerHTML = "";
  },

  events: {
    'click .btn-primary': 'clickSubmit'
  }
});
